 ...
 *
 *  - Pace            words / actual speaking time (articulation rate)
 *  - Pauses          silence detection via an adaptive energy threshold
 *  - Energy dynamics how much loudness varies (monotone vs. expressive volume)
 *  - Intonation      pitch (F0) variation via autocorrelation on voiced frames
 *
 * The goal is "good enough to coach", not forensic accuracy. All heavy loops
 * are bounded so a 5-minute clip analyses in well under a second.
 */
...
export async function analyzeAudio(blob: Blob, wordCount?: number): Promise<AudioMetrics> {
  ...
  try {
    ...
    // 3) Walk frames to find speaking time and pauses.
    ...
    // Trailing silence is ignored (it's just dead air at the end of the take).
    ...
    // 4) Energy dynamics in dB over voiced frames.
    ...
    // 5) Pitch tracking on a bounded sample of voiced frames.
    const maxPitchFrames = 1200; // keep total work bounded for long clips
    ...
    for (let f = 0; f < frameCount; f += step) {
      ...
      if (f0 >= MIN_F0 && f0 <= MAX_F0) f0s.push(f0);
    }
    ...
    // Convert each F0 to semitone distance from the median for variation stats.
